  ...
  return (
    <div className="p-6">
      ...
      <div className={`${card} p-4 mb-4`}>
        <p className={`text-xs font-semibold mb-2 ${d ? "text-zinc-400" : "text-gray-500"}`}>TIPOS DE EVENTO</p>
        <div className="flex gap-4 flex-wrap">
          {[{ label: "Culto", color: "#2563eb" }, { label: "Ministerio", color: "#a855f7" }, { label: "Pastoral", color: "#22c55e" }, { label: "Jóvenes", color: "#f59e0b" }].map(c => (
            <div key={c.label} className="flex items-center gap-2 text-sm">
              ...
              <span className={d ? "text-zinc-300" : "text-gray-600"}>{c.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`${card} p-4`}>
        <FullCalendar
          ...
          locale="es"
          events={eventos.map(e => ({ id: e.id, title: e.title, date: e.date, backgroundColor: e.color, borderColor: e.color }))}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
          <div className={`border rounded-2xl p-6 w-full max-w-md mx-4 ${d ? "bg-zinc-800 border-zinc-700" : "bg-white border-gray-200"}`}>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setDetalleModal(false); abrirEditar({ event: { id: eventoDetalle.id } }); }}
              ...
              <button onClick={() => setDetalleModal(false)}
                className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-2 rounded-lg transition">
                Cerrar
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
          <div className={`border rounded-2xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto ${d ? "bg-zinc-800 border-zinc-700" : "bg-white border-gray-200"}`}>
              ...
              <div>
                  ...
                  <div className={`rounded-lg p-3 mb-3 space-y-2 ${d ? "bg-zinc-700" : "bg-gray-50"}`}>
                    {form.roles.map((r, i) => (
                      <div key={i} className="flex items-center justify-between">
                        ...
                        <button onClick={() => quitarRol(i)} className="text-red-400 hover:text-red-500">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    ))}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={nuevoRol.tarea}
                    onChange={e => setNuevoRol({ ...nuevoRol, tarea: e.target.value })}
                    placeholder="Tarea (Aseo, Sonido...)"
                  />
                  <input
                    className={`${input} flex-1`}
                    value={nuevoRol.persona}
                    onChange={e => setNuevoRol({ ...nuevoRol, persona: e.target.value })}
                    placeholder="Nombre"
                  />
                  <button onClick={agregarRol} className="bg-blue-600 hover:bg-blue-500 text-white px-3 rounded-lg transition">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
            ...
            <div className="flex gap-3 mt-6">
              {eventoSeleccionado && (
                <button onClick={eliminar} className="flex-1 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-sm font-medium py-2 rounded-lg transition">
                  Eliminar
                </button>
              )}
              <button onClick={guardar} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-2 rounded-lg transition">
