...
export default function App() {
  ...
  const opcionesMenu = esAdmin 
    ? [
        { icon: Home, label: "Dashboard", id: "dashboard" },
        { icon: Clock, label: "Mi Servicio", id: "servicio" },
  ...
  const eventosEsteMes = listaEventos.filter(e => e.date.split("-")[1] === "05").length;
  const proximosEventosFormateados = [...listaEventos].sort((a, b) => new Date(a.date) - new Date(b.date)).slice(0, 4);

  const stats = [
    { label: "Miembros activos", value: "124", icon: Users, color: "bg-blue-600" },
    { label: "Eventos este mes", value: eventosEsteMes.toString(), icon: Calendar, color: "bg-blue-500" },
    { label: "Peticiones nuevas", value: "13", icon: Heart, color: "bg-blue-700" },
    { label: "Avisos enviados", value: "5", icon: Bell, color: "bg-blue-400" },
  ];

  const formatearFechaDashboard = (fechaStr) => {
    const diasSemana = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
    const mesesAnio = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];
    const fecha = new Date(fechaStr + "T00:00:00");
    return `${diasSemana[fecha.getDay()]} ${fecha.getDate()} ${mesesAnio[fecha.getMonth()]}`;
  };

  return (
    <div className={`min-h-screen flex ${d ? "dark bg-zinc-900 text-white" : "bg-gray-100 text-gray-900"}`}>
      <aside className={`fixed z-20 top-0 left-0 h-full w-64 flex flex-col transition-transform duration-300 ${menuAbierto ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:static ${d ? "bg-zinc-800 border-r border-zinc-700" : "bg-white border-r border-gray-200"}`}>
        <div className={`p-6 border-b flex flex-col items-center gap-3 ${d ? "border-zinc-700" : "border-gray-200"}`}>
          ...
          <div className="text-center">
            ...
            <p className={`text-xs mt-0.5 ${d ? "text-zinc-400" : "text-blue-400"}`}>{esAdmin ? "Panel de administración" : "Panel del Servidor"}</p>
          </div>
        </div>
        ...
        <div className={`p-4 border-t flex flex-col gap-3 ${d ? "border-zinc-700" : "border-gray-200"}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className={`text-sm font-medium ${d ? "text-white" : "text-gray-800"}`}>{usuario.nombre}</p>
              <p className={`text-xs ${d ? "text-zinc-400" : "text-gray-400"}`}>{esAdmin ? "Administrador" : "Servidor"}</p>
            </div>
          </div>
          <button onClick={() => setUsuario(null)} className="w-full text-left text-xs text-red-500 hover:text-red-400 font-medium transition cursor-pointer mt-1">Cerrar Sesión</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        <header className={`border-b px-6 py-4 flex items-center justify-between ${d ? "bg-zinc-800 border-zinc-700" : "bg-white border-gray-200 shadow-sm"}`}>
          <div className="flex items-center gap-3">
            <button className="md:hidden" onClick={() => setMenuAbierto(!menuAbierto)}>{menuAbierto ? <X size={22} /> : <Menu size={22} />}</button>
            <div>
              <h2 className={`text-lg font-semibold ${d ? "text-white" : "text-gray-800"}`}>{pagina.charAt(0).toUpperCase() + pagina.slice(1)}</h2>
              <p className={`text-xs ${d ? "text-zinc-400" : "text-gray-400"}`}>Miércoles, 20 de mayo de 2026</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setOscuro(!oscuro)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border transition ${d ? "border-zinc-600 text-zinc-300 hover:bg-zinc-700" : "border-gray-300 text-gray-600 hover:bg-gray-100"}`}>{d ? <Sun size={14} /> : <Moon size={14} />}{d ? "Modo claro" : "Modo oscuro"}</button>
            <span className={`text-xs px-3 py-1 rounded-full font-medium ${d ? "bg-blue-600/30 text-blue-300" : "bg-blue-100 text-blue-700"}`}>Estado: Activo</span>
          </div>
        </header>
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.map(({ label, value, icon: Icon, color }) => (
                <div key={label} className={`border rounded-xl p-4 flex items-center gap-4 ${d ? "bg-zinc-800 border-zinc-700" : "bg-white border-gray-200 shadow-sm"}`}>
                  <div className={`${color} p-3 rounded-lg`}><Icon size={20} className="text-white" /></div>
                  <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className={`border rounded-xl p-5 ${d ? "bg-zinc-800 border-zinc-700" : "bg-white border-gray-200 shadow-sm"}`}>
                <h3 className={`text-sm font-semibold mb-4 flex items-center gap-2 ${d ? "text-zinc-200" : "text-gray-700"}`}><Calendar size={16} className="text-blue-500" /> Próximos eventos</h3>
                <div className="space-y-3">
                  {proximosEventosFormateados.map((e) => (
                    <div key={e.id} className={`flex items-center justify-between py-2 border-b last:border-0 ${d ? "border-zinc-700" : "border-gray-100"}`}>
                      <div>
                        <p className="text-sm font-medium">{e.title}</p>
                        <p className={`text-xs ${d ? "text-zinc-400" : "text-gray-400"}`}>{formatearFechaDashboard(e.date)}</p>
                      </div>
              ...
              <div className={`border rounded-xl p-5 ${d ? "bg-zinc-800 border-zinc-700" : "bg-white border-gray-200 shadow-sm"}`}>
                <h3 className={`text-sm font-semibold mb-4 flex items-center gap-2 ${d ? "text-zinc-200" : "text-gray-700"}`}><BarChart2 size={16} className="text-blue-500" /> Asistencia 2026</h3>
                <div className="flex items-end gap-1.5 h-36 px-1">
                  {asistencia.map((val, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full bg-blue-500 rounded-t opacity-80 hover:opacity-100 transition" style={{ height: `${(val / 250) * 144}px` }} title={`${meses[i]}: ${val}`} />
